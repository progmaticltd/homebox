...
if [ "$USE_XMPP" = "1" ]; then
    xmppOutput=$(printf "%s" "$MSG" | sendxmpp -t -f "$xmppConfig" "$MAIL" 2>&1)
...
printf "%s" "$MSG" | mail -a "$contentHeader" -a "$alertHeader" -r "$from" -s "$subject" "$MAIL"
...
if [ "$ALERT_ADDRESS" != "" ]; then
    ...
    if [ "$USE_XMPP" = "1" ]; then
        ...
        xmppOutput=$(printf "%s" "$MSG" | sendxmpp -t -f "$xmppConfig" "$ALERT_ADDRESS")
    fi
    ...
    printf "%s" "$MSG" | mail -a "$contentHeader" -a "$alertHeader" -r "$from" -s "$subject" "$ALERT_ADDRESS"
