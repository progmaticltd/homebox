...
buildAndSendReport() {
    ...
    timeColumns="strftime('%d', min(unixtime)) as startTime,strftime('%H:%M', min(unixtime))"
    timeColumns="$timeColumns, strftime('%d', max(unixtime)),strftime('%H:%M', max(unixtime))"

    if [ "0$nbAddresses" -ge "1" ]; then
        fields="provider,$timeColumns,countryName,count(*) as count"
        ...
        title="<h2>Per Internet Service Provider</h2>"
        headers="<table><th>ISP</th><th colspan='2'>From</th><th colspan='2'>Until</th><th>Country</th><th>Count</th>"
        footer="</table>"
    ...
    if [ "0$nbAddress" -ge "1" ]; then
        ...
        headers="<table><th colspan='2'>From</th><th colspan='2'>Until</th><th>ISP</th><th>Count</th>"
        footer="</table>"
        report="$report $title $headers $addressReport $footer"
        sendReport=1
    fi
    ...
    if [ "0$nbError" -ge "1" ]; then
        fields="strftime('%d', unixtime), strftime('%H:%M', unixtime),ip,countryName,source,lower(status),details"
